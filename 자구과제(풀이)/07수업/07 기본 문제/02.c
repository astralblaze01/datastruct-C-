...
void initMark(int **mark, location markSize);

void push(element inValue);
void printStack();
element pop();
void peek();
int isEmpty(int top);
...
void path(int maze[MAZE_SIZE][MAZE_SIZE], int **mark, offsets move[], location start, location end, location mazeSize);
...
int main()
{
    ...
    // 경로 찾기
    path(maze, mark, move, start, end, mazeSize);
